...
IF ~~ BELordSelemchant0
  SAY @3008
  IF ~~ DO ~SetGlobal("BEArenQuest","GLOBAL",4)~ EXIT
END
...
IF ~PartyHasItem("BEMEDAL")~ THEN BEGIN BELordSelemchantEnd1
   SAY @3009
   IF ~~ DO ~SetGlobal("BEArenQuest","GLOBAL",9) EscapeArea()~ EXIT
END

IF ~Global("BEArenQuest","GLOBAL",12)~ THEN BEGIN BELordSelemchantBad
   SAY @3010
   IF ~~ DO ~SetGlobal("BESelemchantAmbush","GLOBAL",1) EscapeArea()~ EXIT
END